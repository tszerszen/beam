...
const (
	runtimeMetricNamespace = "MeasureTime"
...
func init() {
	beam.RegisterType(reflect.TypeOf((*MeasureTime)(nil)).Elem())
}

// MeasureTime is a DoFn to record processing time in the pipeline.
//
...
func (fn *MeasureTime) StartBundle(ctx context.Context, emit func([]byte, []byte)) {
	runtime.Update(ctx, time.Now().Unix())
}

// FinishBundle updates a distribution metric.
func (fn *MeasureTime) FinishBundle(ctx context.Context, emit func([]byte, []byte)) {
	runtime.Update(ctx, time.Now().Unix())
}

// ProcessElement emits unmodified input elements.
func (fn *MeasureTime) ProcessElement(key []byte, value []byte, emit func([]byte, []byte)) {
	emit(key, value)
}
...
func newLoadTestResult(value float64) loadTestResult {
	...
	if *namespace == "" {
		...
		metric = fmt.Sprintf("%v_%v", *namespace, runtimeMetricName)
	}
...
func PublishMetrics(ctx context.Context, results metrics.QueryResults) {
...
func toLoadTestResults(results metrics.QueryResults) []loadTestResult {
	...
	for _, dist := range results.Distributions() {
		if strings.Contains(dist.Key.Namespace, runtimeMetricNamespace) &&
...
// extractRuntimeValue finds the minimum of minimum values and the maximum of maximum values
// and returns their difference.
func extractRuntimeValue(matchedDist []metrics.DistributionResult) float64 {
	min := matchedDist[0].Result().Min
	max := min

	for _, dist := range matchedDist {
		result := dist.Result()
		if min > result.Min {
			min = result.Min
		}
