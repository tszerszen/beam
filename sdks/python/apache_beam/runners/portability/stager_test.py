...
class StagerTest(unittest.TestCase):
  ...
  def test_sdk_location_default(self):
    ...
    self.assertEqual([stager.WORKFLOW_TARBALL_FILE], staged_resources)

    with open(os.path.join(staging_dir, stager.WORKFLOW_TARBALL_FILE)) as f:
  ...
  def test_sdk_location_default_with_wheels(self):
    ...
    with mock.patch(
      ...
      self.assertEqual(staged_resources[0], stager.WORKFLOW_TARBALL_FILE)
  ...
  def test_sdk_location_local_directory(self):
    ...
    self.create_temp_file(
        os.path.join(sdk_location, stager.WORKFLOW_TARBALL_FILE),
        'Package content.')
    ...
    self.assertEqual([stager.WORKFLOW_TARBALL_FILE],
                     self.stager.create_and_stage_job_resources(
                         options, staging_location=staging_dir)[1])
    tarball_path = os.path.join(staging_dir, stager.WORKFLOW_TARBALL_FILE)
